...
async function createNotification(userId, type, message, metadata = {}) {
  ...
  // Performance Optimization: increment user's denormalized unread count
...
async function notifyGroupPost(groupId, postId, postAuthorId) {
  ...
  const recipients = (group.members || []).filter(mid => mid !== postAuthorId);
  if (!recipients.length) return [];

  const now = new Date();
  const docs = recipients.map(memberId => ({
    _id: uuidv4(),
    userId: memberId,
    type: NOTIFICATION_TYPES.GROUP_POST,
  ...
  // Batch insert and batch increment counts
  const results = await Promise.all([
    Notification.insertMany(docs),
    User.updateMany({ _id: { $in: recipients } }, { $inc: { unreadCount: 1 } })
  ]);
