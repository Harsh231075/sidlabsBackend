...
/**
 * Get list of all blocked user IDs (blocked by me + who blocked me)
 */
async function getAllBlockedUserIds(userId) {
  const [blocked, whoBlocked] = await Promise.all([
    getBlockedUsers(userId),
    getUsersWhoBlocked(userId)
  ]);
  return [...new Set([...blocked, ...whoBlocked])];
}
