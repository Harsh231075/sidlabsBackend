...
function generateToken(user) {
  return jwt.sign(
    { id: user.id || user._id, role: user.role, email: user.email, name: user.name, createdAt: user.createdAt },
    JWT_SECRET,
    { expiresIn: '7d' }
  );
}
...
async function optionalAuth(req, res, next) {
  ...
  try {
    if (authProvider === 'cognito') {
      // Try Cognito auth (with Redis cache)
      const { verifyCognitoToken, extractUserFromPayload } = require('./cognitoAuth');
      const User = require('../models/User');
      const cache = require('../services/cacheService');
      const AUTH_CACHE_TTL = Number(process.env.AUTH_CACHE_TTL || 120);

      const payload = await verifyCognitoToken(token);
      const cognitoInfo = extractUserFromPayload(payload);

      const cacheKey = `auth:sub:${cognitoInfo.cognitoSub}`;
      const user = await cache.getOrSet(cacheKey, async () => {
        const u = await User.findOne({ cognitoSub: cognitoInfo.cognitoSub }).lean();
        return u || null;
      }, AUTH_CACHE_TTL);
