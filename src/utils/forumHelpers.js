...
function buildThreadResponse(thread, users, currentUserId, posts = [], groups = []) {
  ...
  return {
    ...thread,
    creator: creator ? { id: creator.id, name: creator.name, role: creator.role, avatarUrl: creator.avatarUrl } : null,
    replyCount,
    group: group
...
function buildForumPostResponse(post, users) {
  ...
  return {
    ...post,
    author: author ? { id: author.id, name: author.name, role: author.role, avatarUrl: author.avatarUrl } : null,
