...
async function authenticateHybrid(req, res, next) {
  ...
  if (useCognito) {
    try {
      ...
      const AUTH_CACHE_TTL = Number(process.env.AUTH_CACHE_TTL || 120); // 2 min

      // ── Redis-cached user lookup ─────────────────────────────
      const cacheKey = `auth:sub:${userInfo.cognitoSub}`;
      let dbUser = await cache.getOrSet(cacheKey, async () => {
        const u = await User.findOne({ cognitoSub: userInfo.cognitoSub }).lean();
        return u || null;
      }, AUTH_CACHE_TTL);

      if (!dbUser) {
        // Try to link by email (rare, first-time only)
