...
const VERIFY_TTL = 60000; // 1 minute cache
...
async function verifyEmailTransport() {
  ...
  const now = Date.now();
  if (lastVerifyResult && (now - lastVerifyTime < VERIFY_TTL)) {
    return { ...lastVerifyResult, cached: true };
  }

  try {
    await transporter.verify();
    lastVerifyResult = { ok: true, problems: [], cfg };
    lastVerifyTime = now;
    return lastVerifyResult;
  } catch (err) {
    // Don't cache errors for long, maybe 10s? OR don't cache at all.
    // Let's not cache errors so we can retry.
    return {
      ok: false,
