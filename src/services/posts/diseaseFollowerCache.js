...
async function getDiseaseFollowerIdsCached(diseasePageSlug) {
  ...
  // Performance: Limit to avoid memory overflow for massive pages
  const followers = await DiseaseFollower.find({ diseasePageSlug: key }).select('userId').limit(2000).lean();
