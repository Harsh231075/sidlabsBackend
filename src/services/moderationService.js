...
/**
 * Utility to calculate trust score from account age without DB query
 */
function calculateTrustScoreFromDate(createdAt) {
  let trustScore = 0.7;
...
async function scan({ text, userId, context = {} }) {
  ...
  // Optimization: use passed user.createdAt if available to avoid extra DB query
  const userCreatedAt = context.userCreatedAt || (context.user?.createdAt) || null;
  const userTrustScore = (userId && userCreatedAt)
    ? calculateTrustScoreFromDate(userCreatedAt)
    : (userId ? await getUserTrustScore(userId) : 0.5);
  ...
  // Moderate risks → ALLOW (was SOFT_BLOCK)
  // We are relaxing this to avoid false positives on legitimate sharing.
  else if (
    scores.phi_score > 0.6 ||
    scores.spam_score > 0.7 ||
    scores.link_risk_score > 0.8
  ) {
    status = 'QUARANTINE';
  }
