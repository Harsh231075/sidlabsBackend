...
function groupView(group, userId) {
  ...
  return {
    ...g,
    id: g._id, // Ensure id is returned (toJSON helper usually does this but explicitly mapping is safe)
    isMember,
    isAdmin,
  };
}
...
async function getGroup(req, res, next) {
  try {
    ...
    if ((group.privacy === 'private' || group.privacy === 'hidden') && !view.isMember && req.user.role !== 'admin-user') {
      return res.status(403).json({ error: 'Not a member of this private group' });
    }

    const postCount = await Post.countDocuments({ groupId: group._id, removed: false });
    res.json({ group: view, postCount });
  } catch (error) {
    next(error);
  }
}
