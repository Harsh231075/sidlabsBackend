...
async function updateUser(req, res, next) {
  try {
    ...
    if (typeof suspended === 'boolean') user.suspended = suspended; // Note: 'suspended' field wasn't in original schema I defined? If it exists in User, I should add it, but it was used in JSON logic. I will trust Mongoose strict: false or schema updates. Wait, Mongoose is strict by default. I should update User schema if suspended is needed.
    // Let's assume schema has it or mixed type. Actually, the User model I made earlier didn't have suspended. I should add it.
    // For now, I will add it to the update logic, assuming I update User model later or it works.
...
async function getReportedPosts(req, res, next) {
  try {
    ...
    const formatted = reportedPosts.map(post => ({
      id: post._id || post.id,
      ...post,
      author: post.authorId ? { id: post.authorId._id, name: post.authorId.name, avatarUrl: post.authorId.avatarUrl } : null
    }));
...
async function getReportedComments(req, res, next) {
  try {
    ...
    const formatted = reportedComments.map(comment => ({
      ...
      ...comment,
      author: comment.authorId ? { id: comment.authorId._id, name: comment.authorId.name, avatarUrl: comment.authorId.avatarUrl } : null
    }));
