...
async function loadConversationsForUser(userId) {
  ...
  return conversations.map(conv => ({
    ...
    participants: conv.participantIds.map(id => users.find(u => u._id === id)).filter(Boolean).map(u => ({ id: u._id, name: u.name, role: u.role })),
  }));
}

async function checkBlockStatus(userId, otherId) {
  const [iBlockedThem, theyBlockedMe] = await Promise.all([isUserBlocked(userId, otherId), isUserBlocked(otherId, userId)]);
  if (iBlockedThem || theyBlockedMe) {
    let msg = 'Cannot message this user due to a block.';
    if (iBlockedThem && theyBlockedMe) msg = 'Cannot message this user. You have blocked each other.';
    else if (iBlockedThem) msg = 'You have blocked this user. Unblock them to send messages.';
    else msg = 'This user has blocked you. You cannot send them messages.';
...
async function ensureConversation(currentUserId, targetUserId) {
  ...
  const isAdminOrMod = ['admin-user', 'moderator-user'].includes(currentUser?.role) || ['admin-user', 'moderator-user'].includes(targetUser?.role);
  if (!isAdminOrMod) {
    const [friendship, iFollowThem, theyFollowMe] = await Promise.all([
      FriendRequest.findOne({ $or: [{ from: currentUserId, to: targetUserId, status: 'accepted' }, { from: targetUserId, to: currentUserId, status: 'accepted' }] }),
      Follow.exists({ follower: currentUserId, following: targetUserId }),
      Follow.exists({ follower: targetUserId, following: currentUserId }),
    ]);
    if (!friendship && !iFollowThem && !theyFollowMe) throw httpError(403, { error: 'You can only message friends or people you follow/who follow you.' });
  }
  ...
  const participants = existing.participantIds.map(id => participantsUsers.find(u => u._id === id)).filter(Boolean).map(u => ({ id: u._id, name: u.name, role: u.role }));
...
async function sendMessage(userId, convId, text, image) {
  ...
  for (const pid of conv.participantIds) {
    if (pid !== userId) await checkBlockStatus(userId, pid);
  }
