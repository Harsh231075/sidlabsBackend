...
function groupView(group, userId) {
  ...
  // Performance: Use .includes() for strings, fallback to some/toString for safety
  const isMember =
...
async function getGroups(userId, userRole, query) {
  ...
  const filter = { parentGroupId: null };

  if (search) {
    filter.$or = [
      { name: { $regex: search, $options: 'i' } },
      { description: { $regex: search, $options: 'i' } },
      { diseaseTag: { $regex: search, $options: 'i' } }
    ];
  }

  // Admin users looking at the paginated admin view should see all groups
  if (!(isPaginated && userRole === 'admin-user')) {
    const accessFilter = {
      $or: [
        { privacy: { $ne: 'hidden' } },
        { members: userId },
  ...
  if (isPaginated) {
    ...
    const mapped = groups.map(g => groupView(g, userId));
    return {
      data: mapped,
      totalCount: total,
      page,
    ...
    const groups = await Group.find(filter).sort({ createdAt: -1 }).limit(200).lean();
    return groups.map(g => groupView(g, userId));
  }
}
...
async function joinGroup(groupId, userId) {
  ...
  if (!group) {
    ...
    if (!check) throw httpError(404, { error: 'Group not found' });
    return groupView(check, userId); // Already a member
  }
...
async function leaveGroup(groupId, userId) {
  ...
  const updated = await Group.findOneAndUpdate(
    ...
    { $pull: { members: userId, adminIds: userId }, $inc: { memberCount: -1 } },
    { new: true }
  );
  if (!updated) return groupView(group, userId); // Already not a member
  return groupView(updated, userId);
}
